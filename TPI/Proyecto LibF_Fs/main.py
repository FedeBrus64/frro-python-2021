...
def registroCliente():
    if request.method == 'POST':
        ...
        msg= Message('Bienvenido. Gracias por registrarte', sender='libreriafyf@gmail.com', recipients = [email_cli])
...
def contacto():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute('INSERT INTO consulta (usuario_Consulta, descripcion_Consulta, fecha_Consulta) VALUES (%s, %s, current_date())',
        (usuEnSesion, descripcion_Consulta))
        mysql.connection.commit()
...
def mostrarCompras():
    ...
    cur.execute('SELECT fecha_Compra, nombre_Libro, precio_Total_compra FROM compra JOIN libro ON id_Libro_Compra = libro.id_Libro JOIN cliente ON id_Cliente_Compra = cliente.nro_cli WHERE cliente.usuario_cli = %s', (usuEnSesion))
...
def comprarLibro():
    ...
    if request.method == 'POST':
        ...
        mysql.connection.commit()
        cur.execute('INSERT INTO compra (fecha_Compra, usuario_Comprador, libro_Comprado) VALUES (current_date(), %s, %s) ', (usuEnSesion, nombre_lib))

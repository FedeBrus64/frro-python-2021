...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    return sum(x for x in list(map(lambda x: x**3, numeros)) if x % 2 == 0) 
...
numeros_al_cubo_pares = list(x**3 for x in filter(lambda x: x % 2 == 0, numeros))
...
suma_numeros_al_cubo_pares = reduce(lambda x,y: x + y, list(x**3 for x in filter(lambda x: x % 2 == 0, numeros)))
